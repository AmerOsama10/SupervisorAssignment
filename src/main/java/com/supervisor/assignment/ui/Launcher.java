...
public class Launcher extends Application {
	...
	public void start(Stage stage) {
		...
		btnTemplate = new Button("تحميل القالب");
		btnRun.setOnAction(e -> onRun(stage));
		btnTemplate.setOnAction(e -> onGenerateTemplate(stage));
		...
		filterType.getItems().addAll("مشرف", "مادة", "تاريخ");
	...
	private void switchLanguage(Locale locale, Stage stage) {
		...
		btnTemplate.setText("تحميل القالب");
	...
	private void onPrint(Stage stage) {
		...
		if (dest == null) return;
		try {
			new PdfExporter().exportArabic(dest, lastResult);
			alert(i18n.t("print.pdf_created", dest.getAbsolutePath()));
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void onGenerateTemplate(Stage stage) {
		FileChooser fc = new FileChooser();
		fc.getExtensionFilters().add(new FileChooser.ExtensionFilter("Excel Files", "*.xlsx"));
		fc.setInitialFileName(i18n.t("template.filename"));
		File out = fc.showSaveDialog(stage);
		if (out == null) return;
		try {
			com.supervisor.assignment.template.TemplateGenerator gen = new com.supervisor.assignment.template.TemplateGenerator();
			gen.generateTemplate(out, new Config());
			alert(i18n.t("template.created", out.getAbsolutePath()));
		} catch (Exception ex) {
			ex.printStackTrace();
			alert(i18n.t("error.template_creation", ex.getMessage()));
		}
